...
function renderProductCards() {
    ...
    if (swiper) {
        swiper.update();
        // Garante que a paginação é renderizada corretamente após a atualização
...
function render() {
    return `
        <style>
            ...
            .btn-shortcut { background: none; border: none; font-size: 1.75rem; cursor: pointer; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); color: var(--cor-texto-secundario); }
            .card-stock-info { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; text-align: center; margin-top: auto; }
            ...
            .card-menu-dropdown { display: none; position: absolute; right: 0; top: 100%; background-color: var(--cor-fundo-principal); border-radius: 8px; box-shadow: var(--sombra-padrao); z-index: 10; overflow: hidden; }
            .card-menu-dropdown.visible { display: block; }
            .card-menu-dropdown button { display: block; width: 100%; padding: 0.5rem 1rem; background: none; border: none; text-align: left; cursor: pointer; }
...
function mount() {
    ...
    // CORREÇÃO: Usa a subscrição simples que é compatível com o Store.js atual

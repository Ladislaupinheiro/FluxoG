...
/**
 * Agrupa a configuração de todos os event listeners da aplicação.
 */
function setupEventListeners() {
    // LISTENERS DE SEGURANÇA
    sel.formAtivacao.addEventListener('submit', handlers.handleAtivacaoLicenca);
    sel.formCriarSenha.addEventListener('submit', handlers.handleCriarSenha);
    sel.formInserirSenha.addEventListener('submit', handlers.handleInserirSenha);
...
function handleConnectionChange() {
    if (navigator.onLine) {
        ...
        setTimeout(() => { sel.offlineIndicator.classList.remove('visible'); }, 3000);
    } else {
